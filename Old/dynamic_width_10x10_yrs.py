...
ds_file_out = 'ModelOutput/SDW_10x10_UnitCheck_100kyr_dx100_V0_yrs.nc'

#%%Load in parameters from txt file
...
q_norm = mg.add_zeros('normalized__discharge', at='node') #in m2/sec (NOT YEARS, conversion is already done)
...
for i in range(nts):
    ...
    mg.at_node['normalized__discharge'][:] = (Qw[:] / w_avg[:]) #m2/yr 
    ...
    for j in range(len(nodes_ordered) - 1, -1, -1):
        ...
        Qw_sec = Qw[j] / sec_per_yr

        #inputs for function to iterate for flow depth
        func_args = (manning_n, ws[j], thetarad, S[j], Qw_sec)
    ...
    mg.at_node['psi_bed'][:] = rhow * g_years * (1 - mg.at_node['shear_stress__partitioning'][:]) / 2 * (1 + mg.at_node['water_surface__width'][:] * np.tan(thetarad) / (mg.at_node['water_surface__width'][:] * np.tan(thetarad) - 2 * mg.at_node['flow__depth'][:]))
    

    #calculate width coefficient for channel bank
    mg.at_node['psi_bank'][:] = rhow * g_years * mg.at_node['shear_stress__partitioning'][:] / 2 * (mg.at_node['water_surface__width'][:] / mg.at_node['flow__depth'][:] * np.sin(thetarad) - np.cos(thetarad))
    ...
    mg.at_node['surface_water__discharge'][:] = mg.at_node['normalized__discharge'][:] 
    ...
    dz_ad = np.zeros(mg.size('node'))
    dz_ad[mg.core_nodes] = Upliftrate_mperyr * space_dt
