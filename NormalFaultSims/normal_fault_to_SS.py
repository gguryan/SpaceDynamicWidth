...
ds_file_out = 'NormalFault_nx50_ss_5.nc'
...
wr[:] = (mg.at_node['drainage_area'] / 10000)**0.5
...
runoff_manual = 1000 #manual multiplier on discharge
...
for i in range(nts):
    ...
    if elapsed_time_yrs %save_interval== 0:
        ...
        print ("wr=","{0:0.2f}".format(round(wr[out_node], 2)), "h=", "{0:0.2f}".format(round(h[out_node], 2)), "H=", "{0:0.2f}".format(round(H_soil[out_node], 2)))
