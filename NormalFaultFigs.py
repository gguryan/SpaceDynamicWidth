...
ds_1 = xr.open_dataset('NormalFaultSims/NormalFault_nx50_ss_4_noSed.nc')
ds_2 = xr.open_dataset('NormalFaultSims/NormalFault_nx50_ss_4.nc')

ds_3 = xr.open_dataset('NormalFaultSims/nx50_normal_fault_FromSS_NoSed.nc')
ds_4 = xr.open_dataset('NormalFaultSims/nx50_normal_fault_FromSS.nc')
...
# Adjust time coordinates of ds_3
ds_3_time_adjusted = ds_3['time'] + ds_1['time'].max()

# Create a new Dataset with adjusted time coordinates
ds_3_adjusted = ds_3.copy()
ds_3_adjusted['time'] = ds_3_time_adjusted

# Concatenate the datasets along the time dimension
ds1 = xr.concat([ds_1, ds_3_adjusted], dim='time')

# Sort the concatenated dataset along the time dimension
ds1 = ds1.sortby('time')
...
# Adjust time coordinates of ds_3
ds_4_time_adjusted = ds_4['time'] + ds_2['time'].max()

# Create a new Dataset with adjusted time coordinates
ds_4_adjusted = ds_4.copy()
ds_4_adjusted['time'] = ds_4_time_adjusted

# Concatenate the datasets along the time dimension
ds2 = xr.concat([ds_2, ds_4_adjusted], dim='time')
...
#set outlet width to zero for plotting purpose since it never erodes
ds1.channel_bedrock__width.loc[dict(x=0,y=0)] = np.nan
ds2.channel_bedrock__width.loc[dict(x=0,y=0)] = np.nan
...
plt.axvline(x=500000, color='darkgrey', linestyle=':')
