...
mg, df = calc_prf_width(ds, plot_time, sf_min_DA)

#%%

time_arr = ds.time.to_numpy()
...
topo1d_outlet = ds.topographic__elevation.sel(x=dx, y=dx)
...
topo1d.plot(color='indigo', label='upstream')
topo1d_25.plot(color='teal', label='mid-channel')
topo1d_outlet.plot(color='goldenrod', label='outlet')
plt.legend(loc='best')
...
plt.figure(dpi=300, figsize=(5,4))
topo_800.plot(cmap='pink', cbar_kwargs={'label': 'Elevation (m)'})
plt.title('Topography at 800kyr')
...
plt.title('Topo at ' + str(plot_time) + ' years')
...
plt.figure()
plt.plot(df['channel_dist'], df['channel_elev'])
...
plt.figure()
plt.plot(df['channel_dist'], df['channel_Er'])
plt.xlabel('Distance Upstream (m)')
...
plt.figure()
imshow_grid(mg, 'topographic__steepest_slope', at='node')
plt.title('Slope')
